...
export const Card = forwardRef<HTMLDivElement, CardProps>(
  (
    ...
    return (
      <div
        ref={ref}
        className={`${variantClasses[variant]} ${paddingClasses[padding]} ${hover && variant !== 'interactive' ? 'hover:shadow-card-hover transition-shadow duration-300' : ''} ${className}`}
        {...props}
      >
        {children}
      </div>
...
export const CardTitle = forwardRef<HTMLHeadingElement, HTMLAttributes<HTMLHeadingElement>>(
  ({ className = '', children, ...props }, ref) => (
    <h3 ref={ref} className={`font-display text-lg font-semibold text-dark-900 ${className}`} {...props}>
      {children}
    </h3>
  )
);
...
export const CardDescription = forwardRef<HTMLParagraphElement, HTMLAttributes<HTMLParagraphElement>>(
  ({ className = '', children, ...props }, ref) => (
    <p ref={ref} className={`mt-1 text-sm text-dark-500 ${className}`} {...props}>
      {children}
    </p>
  )
);
